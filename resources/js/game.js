...
class Game {
  ...
  setupInteraction () {
    const animationElement = document.querySelector('.canvas--animation')

    if (lib.isMobile()) {
      animationElement.addEventListener('touchstart', (e) => this.handleClickEvent(e.changedTouches[0].clientX, e.changedTouches[0].clientY))
    } else {
      animationElement.addEventListener('click', (e) => this.handleClickEvent(e.clientX, e.clientY))
    }
  }
  ...
  initLevel () {
    ...
    this.dialogueHandler.setCallback(() => {
      ...
      if (this.level.redirectors) {
        this.level.redirectors.forEach(r => {
      ...
      if (this.level.walls) {
        this.addWalls(this.level.walls)
      }
  ...
  handleLevelClear () {
    ...
    fetch(`http://localhost:8080/api/level/${this.level.data.name}/score/time/`, {
      method: 'post',
      headers: {
      ...
      body: JSON.stringify({
        user: 'Martin',
        points: parseInt(Data.instance.elapsedTime)
      })
    })

    fetch(`http://localhost:8080/api/level/${this.level.data.name}/score/bounces/`, {
      method: 'post',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({
        user: 'Martin',
        points: Data.instance.bounces
      })
    })

    fetch(`http://localhost:8080/api/level/${this.level.data.name}/score/redirects/`, {
      method: 'post',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({
        user: 'Martin',
        points: Data.instance.redirects
      })
    })

    fetch(`http://localhost:8080/api/level/${this.level.data.name}/score/total/`, {
      method: 'post',
      headers: {
        'Content-Type': 'application/json'
      },
