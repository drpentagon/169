...
class LevelValidator {
    ...
    validateData(data_) {
    	if(!data_) 
    		return this.addToProtocol("Level has no data");
	...
	validateName(name_) {
    	if(!name_ || name_.length === 0) 
    		return this.addToProtocol("Level name not set");
	...
	validateTimeout(timeout_) {
    	if(!timeout_) 
    		return this.addToProtocol("Level timout not set");

    	if(typeof timeout_ !== "number")
    		return this.addToProtocol("Invalid timeout");

    	if(timeout_ > 120)
    		return this.addToProtocol("Timeout should be a number between 0 and 120");    	
	}

	validateBounceLimit(bounceLimit_) {
    	if(!bounceLimit_) 
    		return this.addToProtocol("Bounce limit not set");

    	if(typeof bounceLimit_ !== "number")
    		return this.addToProtocol("Invalid bounce limit");
	}

	validateRedirectorLimit(redirectorLimit_) {
    	if(!redirectorLimit_) 
    		return this.addToProtocol("Redirector limit not set");

    	if(typeof redirectorLimit_ !== "number")
    		return this.addToProtocol("Invalid redirector limit");
	}

    validateGoals(goals_) {
    	if(!goals_ || goals_.length === 0) 
    		return this.addToProtocol("Level must have at least one goal tile");

    	if(!(goals_ instanceof Array)) 
			return this.addToProtocol('Goals should be defined as an array');
    ...
    validateWalls(walls_) {
		if(!walls_ || walls_.length === 0) 
			return;

		if(!(walls_ instanceof Array)) 			
			return this.addToProtocol('Walls should be defined as an array');
    ...
    validateWallBlock(wallBlock_) {
		...
		if(wallBlock_[0][0] > wallBlock_[1][0])
			this.addToProtocol(`Wall block should be defined from left to right`);

		if(wallBlock_[0][1] > wallBlock_[1][1])
			this.addToProtocol(`Wall block should be defined from top to bottom`);		
    }
    ...
    validateBalls(balls_) {
    	if(!balls_ || balls_.length === 0) 
    		return this.addToProtocol("Level must have at least one ball");

		if(!(balls_ instanceof Array)) 
			return this.addToProtocol('Balls should be defined as an array');
    ...
    validateBall(ball_) {
    	...
    	if(!ball_.dx && !ball_.dy)
    		return this.addToProtocol("Ball must have a speed");
    ...
    validateRedirectors(redirectors_) {
    	if(!redirectors_ || redirectors_.length === 0) 
    		return;

		if(!(redirectors_ instanceof Array))
			return this.addToProtocol('Redirectors should be defined as an array');    	
    ...
    validateRedirector(redirector_) {
    	...
    	if(redirector_.static === undefined)
    		this.addToProtocol(`Redirector static value not set`); 
    }
